# 🏨 Prototype - A Pension Story

[![Unity Version](https://img.shields.io/badge/Unity-2021.3.18f1-blue.svg?style=flat&logo=unity)](https://unity.com/)
[![Language](https://img.shields.io/badge/Language-C%23-green.svg?style=flat&logo=c-sharp)](https://learn.microsoft.com/en-us/dotnet/csharp/)
[![Render Pipeline](https://img.shields.io/badge/Pipeline-URP-orange.svg?style=flat)](https://docs.unity3d.com/6000.4/Documentation/Manual/urp/urp-introduction.html)

A strategic, grid-based hotel management simulator prototype. Take charge of your hospitality business, design optimal floor plans, maintain clean facilities, and expand your infrastructure to climb the global rankings.

---

## 🚀 Quick Start

Want to skip the code and just play?

1. **[Download the Repository](https://github.com/Frext/PROTOTYPE-APensionStory/archive/refs/heads/main.zip)**
2. Extract the ZIP file.
3. Open the **`BuildWindows`** folder and run the game executable named **`PROTOTYPE-APensionStory.exe`**.

---

## 🎮 Controls & Interface

| Interaction | Action |
| :---: | :--- |
| **Left Click** | Manage rooms. |
| **Hold Left Click & Drag** | Pan across the hostel. |

---

## 🎬 Gameplay Preview

https://github.com/user-attachments/assets/14c04340-913f-46bf-afdd-9c9decd231c6

---

## 📸 Screenshots

| Game Start | Gameplay |
|:---:|:---:|
| <img src="https://github.com/user-attachments/assets/fe3ee4d3-ad69-4164-8123-bb7738f2cd64" alt="Game Start"/> | <img src="https://github.com/user-attachments/assets/2f93dbd0-e472-40c4-84f1-83c23e92939e" alt="Gameplay"/> |

| Add New Floors | Modular Room Insertion |
|:---:|:---:|
| <img src="https://github.com/user-attachments/assets/35951df0-d55d-4d3e-a00f-c13cc7d256fa" alt="Add New Floors"/> | <img src="https://github.com/user-attachments/assets/a42f0ca4-bd5a-444d-9170-04803afbeaec" alt="Modular Room Insertion"/> |

| Modular Room Deletion | Leaderboard |
|:---:|:---:|
| <img src="https://github.com/user-attachments/assets/1ec20772-280f-4934-96d0-1e78ff1b6e3b" alt="Structural Room Deletion"/> | <img src="https://github.com/user-attachments/assets/3f66ac4a-69a9-4e11-b3fc-2c3c71faa702" alt="Leaderboard"/> |

---

## 🎯 Core Features

* **Dynamic Structural Expansion:** Seamlessly add new vertical building layers (**Yeni Kat Ekle**) and build vital facilities (**Yeni Oda Ekle**) like Bedrooms (Oda), Kitchens (Mutfak), Receptions (Resepsiyon), and Restrooms (WC/Banyo).
* **Maintenance & Cleaning Loop:** Monitor room conditions actively. Use the contextual broom icon alerts to clean up dirt and reset high-occupancy areas.
* **Layout Modification:** Toggle construction modes (**Odaları silmeyi aç / kapat**) to strip down or redesign rooms for optimal room layouts.
* **Economic Loop & Leaderboard:** Track Daily Income (**Günlük Kazanç**), boost overall Company Value (**Şirket Değeri**), and compare your business prowess against other firms on the live scoreboard (**Skor Tablosu**).

---

## 🛠️ Built With

* **Game Engine:** Unity (2021.3.18f1)
* **Scripting Language:** C#
* **Graphics Pipeline:** Universal Render Pipeline (URP)

---

**2023**
